...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    hidden_pre = X @ W1 + b1.view(1 , -1)
    hidden = hidden_pre.clamp( min = 0) # clamp: giving a range on min / max
    scores = hidden @ W2 + b2.view( 1 , -1)
    # easyto think by size of matrix
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_forward_backward(
    ...
    # Replace "pass" statement with your code
    # using Softmax
    h1 = X @ W1 + b1    #hidden layer output with shape (N,H)  
    # Apply ReLU activation to h1
    h1_relu = torch.maximum(torch.tensor(0,device=h1.device,dtype=h1.dtype),h1)   #relu output with shape (N,H)  

    scores = h1_relu @ W2 + b2     #score with shape (N,C)
    processed_scores = torch.exp(scores - torch.max(scores , dim = 1 , keepdim=True).values)
    probs = processed_scores / torch.sum(processed_scores, dim = 1, keepdim = True)
    loss_everyline = - torch.log(probs[range(N) , y])
    data_loss = loss_everyline / N # better : torch.mean(loss_everyline)

    reg_inloss = reg*(torch.sum(W1 * W1) + torch.sum(W2 * W2))
    loss = data_loss + reg_inloss



    # Now for gradients
    dd = probs.clone()
    dd[range(N),y] = dd[range(N),y] - 1
    dd /= N
    grads = {}
    grads ["W2"] = h1.T @ dd + 2 * reg * W2
    grads ["b2"] = torch.sum(dd , dim = 0)
    dh1 = dd @ W2.T 
    dh1 [h1.relu <= 0] = 0 # Relu process
...
def nn_train(
    ...
    for it in range(num_iters):
        ...
        params ["b1"]  -= learning_rate * grads["b2"]
        # using SGD
        #########################################################################
        #                             END OF YOUR CODE                          #
        #########################################################################
...
def nn_get_search_params():
    ...
    regularization_strengths = [ 1e-1 , 1e0 , 1e1 , 2e1 ]
    learning_rate_decays = [1.0, 0.95, 0.75, 0.5]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def find_best_net(
    ...
    for a in learning_rates:
        for b in hidden_sizes:
            for c in regularization_strengths:
                for d in learning_rate_decays:
                    net = TwoLayerNet(input_size=X_train.shape[1], hidden_size = b ,  )
                    stats = net.train( X_train , y_train , X_val , y_val , learning_rate=a , reg = c , learning_rate_decay = d , num_iters = 1000 ,  batch_size= 128 , verbose = False) # verbose true pointing output detailed data
