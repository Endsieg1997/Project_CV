...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        # Pad input
        x_pad = torch.nn.functional.pad(x, (pad, pad, pad, pad)) # pad参数中那几个对应上下左右填充
        ...
        H_out = 1 + (H + 2 * pad - HH) // stride # 整除 ，往下除法算
            ...
            for f in range(F):  # For each filter
                for i in range(H_out):  # For each output row
                    for j in range(W_out):  # For each output column
                        ...
                        # Extract receptive field and compute convolution
                        out[n, f, i, j] = torch.sum(
                            
                            x_pad[n, :, h_start:h_end, w_start:w_end] * w[f]
                        ) + b[f] # 自己还是很容易卡住 ， channel 这里的索引直接用 : 
                                # out 里的每个元素都是感受范围里卷积出来的结果 ， 
                                # 直觉上会感觉是不是远超范围 --> Normalization 
    ...
    def backward(dout, cache):
        ...
        """
        x, w, b, conv_param, x_pad = cache  # x_pad 写了在 cache  当中
        pad = conv_param['pad']
        stride = conv_param['stride'] 
        ...
        # Compute db
        db = torch.sum(dout, dim=(0, 2, 3)) # 矩阵求导还是对应到维度 一定与求导的变量一致
                                            # 链式传播承上之前的数值 ， 每个卷积核线性卷积后生成dout里会加上 bias
                                            # 一个 filter 刷过一张图横向和纵向 生成一张特征图 然后样本数个这样的图
                                            # b 向量 维度就是几个卷积几个b 所以 dout收缩 直接求和就行因为反向传播环节里就是无非乘个1
                                            # bias 在所有空间位置贡献 “算贡献” 也就是找到所有它参与的地方啦
            ...
            for f in range(F):
                for i in range(H_out):
                    for j in range(W_out):
                        ...
                        # h 和 w 
                        dw[f] += dout[ n , f , i , j ] * x_pad [ n, :, h_start:h_end, w_start:w_end]
                        
                        # In convolution its easier to compute  
                        # No need for dimensionals Reason        
        ...
        # Remove padding 
        dx = dx_pad[:, :, pad:-pad, pad:-pad] # Index itsjust
            ...
            for c in range(C):
                for h in range(H_out):
                    for w in range(W_out):
                        ...
                        #find which makes the max
                        max_x = torch.argmax(window) # mention that it is one dimeonsional ( 3*3 -> row 2 col 1 -> the 7th )





                        h_max , w_max = torch.unravel_index(max_x, (HH , WW) ) 
                        
                        # unravel_index converts the tensor to index of a matrix
                        # 对start 加上window里相对的位置对应回 ， max 函数对提供max值的自变量求导还是1
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        ######################################################################
        # TODO: Initialize weights，bias es for the three-layer convolutional #
        ...
        # biases one dimension equals to total output
        
        # Calculate dimensions after conv and pooling



        # Then 2x2 max pool reduces each dimension by 2
        H_pool = H // 2
        W_pool = W // 2
        ...
        # Initialize first fully-connected layer parameters
        self.params['W2'] = torch.randn(num_filters * H_pool * W_pool, hidden_dim, 
                                      dtype=dtype, device=device) * weight_scale
        self.params['b2'] = torch.zeros(hidden_dim, dtype=dtype, device=device)
        # Initialize second fully-connected layer parameters
    ...
    def loss(self, X, y=None):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        X_1 , cache_1 = Conv.forward( X , W1 , b1 , conv_param ) # 写的时候别忘了 padding stride 这些 parameters 也要用参数
        relu_1 , cache_1_relu = ReLU.forward(X_1)
        X_2 , cache_2 = MaxPool.forward( X_1 , W2 , b2 , pool_param ) 
        N , C , H , W = X_2.shape()
        flat_X = X_2.reshape(N , -1) # 这里是变成两维 N行 ， 列上的数据就直接系统自己算
        fc1_out, fc1_cache = Linear.forward(flat_X, self.params['W2'], self.params['b2'])
        relu2_out, relu2_cache = ReLU.forward(fc1_out) # 
        scores, fc2_cache = Linear.forward(relu2_out, self.params['W3'], self.params['b3'])
        ...
        # notice to add regularization of loss and grads
        reg_loss = 0.0
        for param in ['W1', 'W2', 'W3']:
            reg_loss += torch.sum(self.params[param] ** 2) # L2 regularization
        ...
        dx3, grads['W3'], grads['b3'] = Linear.backward(dscores, fc2_cache)
        grads['W3'] += 2 * self.reg * W3
        
        # ReLU2
        dx2 = ReLU.backward(dx3, relu2_cache)
        
        # 全连接层1
        dx1, grads['W2'], grads['b2'] = Linear.backward(dx2, fc1_cache)
        grads['W2'] += 2 * self.reg * W2
        
        
        # ! shape it back !! 
        dx1 = dx1.reshape(N, C, H, W)
        dpool = MaxPool.backward(dx1, cache_2)
        
        # ReLU1
        drelu = ReLU.backward(dpool,  cache_1_relu)
        
        # 卷积层
        dx, grads['W1'], grads['b1'] = Conv.backward(drelu,  cache_1)
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        for i in range(len(num_filters)):
            ...
            self.params[f'conv{i}_W'] = torch.empty(
                num_filters[i], prev_channels, 3, 3,
                dtype=dtype, device=device
            ) # Using some techniques lllike f '{}'
            self.params[f'conv{i}_W'].data.normal_(mean=0.0, std=weight_scale) # 另一种初始化方
            self.params[f'conv{i}_b'] = torch.zeros(
                num_filters[i], dtype=dtype, device=device
            ) # 同一批上的多个卷积核 一开始对应的频道数都是三 
        ...
        prev_channels = num_filters[i]  # 更新输入通道数        
        pooling_steps = len(self.max_pools)
        H_final = H // (2 ** pooling_steps)
        W_final = W // (2 ** pooling_steps)
        fc_input_dim = num_filters[-1] * H_final * W_final
        ...
        # fc initializationn
        self.params['fc_W'] = torch.empty(
            fc_input_dim, num_classes,
            dtype=dtype, device=device
        )
        self.params['fc_W'].data.normal_(mean=0.0, std=weight_scale)
        
        self.params['fc_b'] = torch.zeros(
    ...
    def loss(self, X, y=None):
        ...
        for i in range( len(self.num_filters) - 1 ):
            ...
            W = self.params[f'conv{i}_W']
            b = self.params[f'conv{i}_b']
            ...
            # Convolutional layer
            conv_out, conv_cache = FastConv.forward(layer_input, W, b, conv_param)

            # Batch normalization (if enabled)
            if self.batchnorm:
                gamma = self.params[f'conv{i}_gamma']
                beta = self.params[f'conv{i}_beta']
                conv_out, bn_cache = SpatialBatchNorm.forward(conv_out, gamma, beta, self.bn_params[i])

            # ReLU activation
            relu_out, relu_cache = ReLU.forward(conv_out)
            
            # Max pooling (if this layer has pooling)
        ...
        flattened_input = layer_input.view(N, -1)

        # Fully connected layer 
        W_fc = self.params['fc_W']
        
        
        b_fc = self.params['fc_b']
        scores, fc_cache = Linear.forward(flattened_input, W_fc, b_fc)
        #####################################################
        #                 END OF YOUR CODE                  #
        ...
        for param in self.params:
            if 'W' in param or 'conv' in param:
                reg_loss += torch.sum(self.params[param] ** 2)
        ...
        loss += self.reg * reg_loss # 先得到，损失值




        # Backpropagation through the fully connected layer
        dx_fc, grads['fc_W'], grads['fc_b'] = Linear.backward(dscores , fc_cache  )
        grads['fc_W'] += 2 * self.reg * W_fc # 正则化

        # Reshape the gradient to match the shape of the last convolutional layer
        dx_fc = dx_fc.view(N, C, H, W)

        # Backpropagation through the convolutional layers
        for i in reversed(range(len(self.num_filters) - 1)):
            # Max pooling (if this layer has pooling)
            if i in self.max_pools:
                dx_fc = FastMaxPool.backward(dx_fc, pool_cache)

            # ReLU activation
            dx_fc = ReLU.backward(dx_fc, relu_cache)

            # Batch normalization (if enabled)
            if self.batchnorm:
                dx_fc, grads[f'conv{i}_gamma'], grads[f'conv{i}_beta'] = SpatialBatchNorm.backward(dx_fc, bn_cache)

            # Convolutional layer
            dx_fc, grads[f'conv{i}_W'], grads[f'conv{i}_b'] = FastConv.backward(dx_fc, conv_cache)
            grads[f'conv{i}_W'] += 2 * self.reg * self.params[f'conv{i}_W']  # 正则化
        #############################################################
        #                       END OF YOUR CODE                    #
        #############################################################
