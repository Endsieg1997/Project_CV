...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        x_1 = x.view(N , -1) 
        out = x_1@w + b
      #不知道prod方法之前想到的是循环着读取x.shape[i] 乘起来
    ...
    def backward(dout, cache):
        ...
        x_1 = x.view(N,-1)
        dx = dout @ w.t()
        dx =  dx.reshape(*x.shape) ## Idontthinklikethat # 一开始感觉不对 ， 但实际上需要匹配原形状才能用
        dw = x_1.t() @ dout
        db = dout.sum(dim = 0) # 还是可以对应着形状去想为何要加起来，BP 矩阵求导都可以这么做
        ##################################################
        #                END OF YOUR CODE                #
        ##################################################
        return dx, dw, db
...
class ReLU(object):
    ...
    def forward(x):
        ...
        ###################################################
        # Replace "pass" statement with your code
        out = x.clamp(min = 0) # clamp 大概是夹子的意思
...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
                 ...
                 dtype=torch.float32, device='cuda'): # device注意是cuda
        """
        ...
        ###################################################################
        # Initialize weights from normal distribution and biases to zero
    ...
    def load(self, path, dtype, device):
        ...
        self.reg = checkpoint['reg'] # 这个函数的参数包括着 dytpe 和 device
    ...
    def loss(self, X, y=None):
        ...
        # Ensure input is on same device and dtype as parameters
        if not X.is_cuda:
            X = X.to(device=self.params['W1'].device)
        ...
        ## ！ 没注意到下面那句话 ， y 就是关于着 是否是测试，要不要反向传播的参数
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        - seed: If not None, then pass this random seed to the dropout
          ...
          # seed is for random dropout so; the same number makes it sustainable to test
        - dtype: A torch data type object; all computations will be
          performed using this datatype. float is faster but less accurate,
          so you should use double for numeric gradient checking.
        - device: device to use for computation. 'cpu' or 'cuda'
        ...
        #######################################################################
        # Replace "pass" statement with your code
        # Initialize parameters on CUDA device
        self.params['W1'] = torch.rand(input_dim, hidden_dims, dtype=dtype, device=self.device) * weight_scale
        self.params['W2'] = torch.rand(hidden_dims, num_classes, dtype=dtype, device=self.device) * weight_scale
        self.params['b1'] = torch.zeros(hidden_dims, dtype=dtype, device=self.device)
        self.params['b2'] = torch.zeros(num_classes, dtype=dtype, device=self.device)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        loss ,dout = softmax_loss (scores , y)
        loss += self.reg * (torch.sum(self.params['W1']**2 )+ torch.sum(self.params['W2']**2))
        dh1 , grads['W2'] = Linear.backward(dout , cache2)
        grads['W2'] += 2 * self.reg * self.params['W2']
        dx , grads['W1'] = Linear_ReLU.backward(dh1 , cache1)
        grads['W1'] += 2 * self.reg * self.params['W1']
        ###########################################################
        #                   END OF YOUR CODE                      #
...
def create_solver_instance(data_dict, dtype, device):
    ...
    # Create mapping from string to actual update rule functions
    update_rules = {
        'sgd': sgd,
    ...
    # Get the actual update rule function
    update_rule_fn = update_rules[solver_config['update_rule']]
    ...
    solver = Solver(model, data_dict,
                    ...
                    optim_config = solver_config['optim_config'],
                    lr_decay = solver_config['lr_decay'],
                    num_epochs = solver_config['num_epochs'],
                    batch_size = solver_config['batch_size'],
                    print_every = solver_config['print_every'],
...
def get_three_layer_network_params():
    ...
    # Replace "pass" statement with your code
    # 手动改一下weight_scale , learning_rate
    ################################################################
    #                             END OF YOUR CODE                 #
    ################################################################
...
def sgd_momentum(w, dw, config=None):
    ...
    v = config.get('velocity', torch.zeros_like(w)) # 找不到可以直接初始化在get里
    ...
    next_w = None
    # Momentum update 
    v = config['momentum'] * v - config['learning_rate'] * dw # 不需要在每次更新中 * （1 - momentum系数） 已经可以跑通
    next_w = w + v
...
def adam(w, dw, config=None):
    ...
    next_w = w - config['learning_rate'] *m_hat / (torch.sqrt(v_hat) + config['episilon'])
    config ['t'] += 1
   ...
   # 从原理上看就是在保持之前的 均值和方差 来更好变动 adam结合了对均值和方差  
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            # Replace "pass" statement with your code
            out = x  
            # during the test just dont use any kind of randomness
            ##############################################################
            #                      END OF YOUR CODE                      #
            ##############################################################
