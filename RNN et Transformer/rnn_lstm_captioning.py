...
class ImageEncoder(nn.Module):
    ...
    def __init__(self, pretrained: bool = True, verbose: bool = True):
        ...
        if verbose: # If outputs the shape information initializing
...
def rnn_step_backward(dnext_h, cache):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    dnext_h1 = (1 - dnext_h**2 ) * dnext_h
    dx = dnext_h1 @ cache["Wx"].T()
    dprev_h = dnext_h1 @ cache["Wh"]
    dWx = cache["x"].T() @ dnext_h1
    dWh = cache["prev_h"].T() @ dnext_h1
    db = dnext_h1.sum( axis = 0 ) * 1 # 也可以对齐维度这样想 
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    h = torch.zeros((N, T, H), device=x.device)
     ...
     # h_prev + h_next -> space for speed
    # Initialize cache dictionary
    cache = {
        'x': x.clone(),  # (N, T, D)
        'h0': h0.clone(),  # (N, H)
        'h_prev': torch.zeros((N, T, H), device=x.device),  # previous hidden states
        'h_next': torch.zeros((N, T, H), device=x.device),  # next hidden states 
        'Wx': Wx,
        'Wh': Wh,
        'b': b
    }
    ...
    prev_h = h0
    for t in range(T):
        # Current input for timestep t (N, D) # 意义在分块理解图像 提取局部特征
        xt = x[:, t, :] # 取出 N * D 
        ...
        cache['h_prev'][:, t, :] = prev_h
        cache['h_next'][:, t, :] = next_h
        prev_h = next_h
...
def rnn_backward(dh, cache):
    ...
    N , T , D = dh.shape
    x , h0 , h_prev , h_next , Wx , Wh , b = cache
    dx = torch.zeros((N, T, D))
    dWx = torch.zeros_like(Wx)
    dWh = torch.zeros_like(Wh)
    db = torch.zeros_like(b)
    dh_prev = torch.zeros((N, D))


    for t in reversed(range(T)):
        dh_current = dh[: , t , :] + dh_prev # 叫成 dh_next 也可以
        dx_step , dh_prev , dWx_step , dWh_step , db_step = rnn_step_backward(dh_current , cache)
...
class WordEmbedding(nn.Module):
    ...
    def __init__(self, vocab_size: int, embed_size: int):
        super().__init__() # super () -> 将找到直接父类 nnmodule
    ...
    def forward(self, x):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        out = self.W_embed[x] # 高级索引： X 中每个元素来自于 W_embed 里的每条行
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        return out
...
def temporal_softmax_loss(x, y, ignore_index=None):
    ...
    Args:
        x: Input scores, of shape (N, T, V)  # 其实就是在输出结果中 D 词向量转换成 V 词当中的可能
...
class CaptioningRNN(nn.Module):
    ...
    neural network. # Captions 就是 对图像 description
    ...
    def forward(self, images, captions):
        ...
        # be the first word. # _in : " START hello my " _out : " hello my friend "
        ...
        # forward 这里是正在进行训练 调整 rnn 的权重 就是要在样本图里提取出 h0 配合随机的 start 词
        # 得到 h1 配合上 h1能生成一个词 比较样本中正确x1 得到loss x1词配合h1也可以得到 h2
        # h2 变成 x2 词然后再比较得到loss
        # 
        #  T 是时间步也可以看作 每个词就是T中的一个步数 T来自于直接设置或者数据样本里长度的 95 %
        # 而推理怎么前进可以自己想想 （ 隐藏状态和隐藏状态推出的词分开 ）


        # nn.module 子类自带有 _call_ 方法给 forward 直接类实例会执行 forward 函数
        ...
        hidden = self.rnn( word_embeds , h0 )

        # _in and _out 都算对句子简单处理 一整个序列转化成向量 在 rnn 当中会生成每个词需要的很多 隐藏向量 然后还有一个点 每次训练时用的词都是样本里来的正确输入 不会有错上加错可能
        scores = self.output_projection ( hidden )
    ...
    def sample(self, images, max_length=15):
        ...
        #######################################################################
        # Replace "pass" statement with your code
        # Extract image features and project to initial hidden state
        features = self.image_encoder(images)
        h = self.feature_projection(features.mean(dim=(2, 3)))
        
        current_word = torch.tensor([self._start] * N, device=images.device)
        captions[:, 0] = current_word
        for t in range(1, max_length):
            # Embed previous word
            word_embed = self.word_embedding(current_word)
            
            # RNN step forward
            h = self.rnn.step_forward(word_embed, h)
            
            # Get scores over vocabulary
...
class LSTM(nn.Module):
    ...
    def step_forward(
        ...
        # Replace "pass" statement with your code
        gates = x @ self.Wx + prev_h @ self.Wh + self.b


        i , f , o , g = torch.split ( gates , gates.size(1) // 4 , dim = 1)
        
        i = torch.sigmoid (i) # 选取多少的信息进入
        f = torch.sigmoid (f) # 遗忘旧信息的比例值
        o = torch.sigmoid (o)

        g = torch.tanh(g) # 新输入可能的所有信息

        next_c = f * prev_c + i * g # 维度均等于 n times h 
    ...
    def forward(self, x: torch.Tensor, h0: torch.Tensor) -> torch.Tensor:
        ...
        for t in range(T):
            x_i = x[ : , t , :]
            next_h , next_c = self.step_forward ( x_i , prev_h , prev_c )
            hn[ : , t , : ] = next_h
            prev_h = next_h
...
def dot_product_attention(prev_h, A):
    ...
    att_score = torch.bmm( A_1.transpose( 1 , 2 ) , prev_h ).squeeze ( 2 )
    ...
    attn = torch.bmm ( A_1 , attn_weights.view( N , - 1 , 1 )).squeeze ( 2 )
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class AttentionLSTM(nn.Module):
    ...
    def step_forward(
        ...
        # Replace "pass" statement with your code
        combined = torch.cat( [x , attn] , dim = 1 ) # ( N*D + N*H = N*(D+H) ) while Stack add one dimension
        gates = (combined @ self.Wx + prev_h @ self.Wh + attn @ self.Wattn + self.b )
        i , f , o , g = torch.split ( gates , gates.size(1) // 4 , dim = 1 )
        i = torch.sigmoid (i)
        f = torch.sigmoid (f)
        o = torch.sigmoid (o)
    ...
    def forward(self, x: torch.Tensor, A: torch.Tensor):
        ...
        # Initialize states from mean pooled features
