...
def prepocess_input_sequence(
    ...
    for token in tokens:
        ...
        if token in spc_tokens:
            ...
            for digit in token:
                out.append(token_dict[digit]) # 数字一个个 digit 这样加
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_two_loop_batch(
    ...
    for n in range(N):
        for i in range(K):
            weights = torch.zeros(K)
            for j in range(K):
                weights[j] = torch.dot(query[n,i], key[n,j]) / (M ** 0.5)
            weights = F.softmax(weights, dim=0)
...
def scaled_dot_product_no_loop_batch(
    ...
    if mask is not None:
        # Apply mask by setting masked positions to -infinity ; 掩码当中设定为 false 对应注意力位置无效所以权重是 极小
...
class SelfAttention(nn.Module):
    def __init__(self, dim_in: int, dim_q: int, dim_v: int):
        ...
        # by [-c, c]                                                             #
        # where c = sqrt(6/(D_in + D_out))  可以数学推导得到用 c 这个范围初始化                                     #
        ...
        c = (6 / (dim_in + dim_q)) ** 0.5 # average forwardandbackward propagation
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        self.heads = nn.ModuleList([
            SelfAttention(dim_in, dim_out, dim_out)  # dim_out 往往会设定为 selfattention 中 dim_out / heads ， 让多个注意力头专注到不同的 low dimension subspace
            for _ in range(num_heads)
        ])  # ModuleList a kind of special container i - nPytorch
    ...
    def forward(
        ...
        for head in self.heads:
            head_out = head(query, key, value, mask)
        ...
        y = self.proj(concat ) # proj 是  layer nn.Linear , 输入参数只需要线性变换的自变量    
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
class FeedForwardBlock(nn.Module): # 前馈神经网络 ， 在 multi - head - attention + addandnormalization 后
    ...
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        # change?                                                                #
        ##########################################################################
        # Initialize first linear layer (expansion)
        ...
        # Initialize second linear layer (contraction)
...
class EncoderBlock(nn.Module):
    def __init__(
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        ...
        self.norm2 = LayerNormalization(emb_dim) #  one for attention ; one for forwardfeed
...
class DecoderBlock(nn.Module):
    ...
    def forward(
        ...
        out1 = self.norm1( dec_inp + attn1 )
        
        attn2 = self.attention_cross(out1, enc_inp, enc_inp) # q , k , v  
...
class Encoder(nn.Module):
    def __init__(
        ...
        self.layers = nn.ModuleList(
            [
                EncoderBlock(num_heads, emb_dim, feedforward_dim, dropout) # Class element filled with initialization parameter ; then the class can be used directly as forward
...
def position_encoding_simple(K: int, M: int) -> Tensor:
 ...
 ## Position encoding 让每个词向量包 ， 括在句子中顺序和相对位置关系的信息
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    encoding = positions.unsqueeze ( 1 ).repeat( 1 , M ) # 先 unsqueeze ， 得到二维向量 ， 有 Row of K , 每行就一个元素从 0 ~ K-1 , repeat是对 Row or Column 重复的次数为几何
    y = encoding.unsqueeze(0)  # 在 0 维加了个维度 Tensor 变成了  （ 1 , K , M )
    ##############################################################################
    #               END OF YOUR CODE                                             #
    return y 
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        a_emb_inp = a_emb[:, :-1] + ans_pos[:, :-1] # 忽略 EOS
        dec_out = None
        ...
        mask = get_subsequent_mask ( ans_b [: , 0 : -1] )
