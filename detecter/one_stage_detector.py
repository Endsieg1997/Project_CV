...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for out_ch in stem_channels:
            ...
            nn.init.normal_(conv.weight, mean=0, std=0.01) # pytorch 中给的要求 deviation 一般就是标准差的设定
        ...
        self.stem_cls = nn.Sequential(*stem_cls) # . sequential 是一种容器 可以将多个子模块 （层） 顺序组合在一起的
        ...
        self.pred_cls = nn.Conv2d(
            in_channels= stem_channels[-1] , # the last layer of network on feature
        ...
        self.pred_box = nn.Conv2d(
            ...
            out_channels= 4, # left top ; right bottom
            kernel_size= 3,
            stride=1,
            padding=1
        )
        ...
        # I thought centerness is computed from formulas but in FCOS they
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for level in ["p3", "p4", "p5"]:
            # Get input features for this level
            features = feats_per_fpn_level[level]
            
            # Process through stems ； sequential 这个容器可以接受张量作为输入
            ...
            cls_logits = self.pred_cls(cls_features)  # (N, C, H, W)
            box_deltas = self.pred_box(box_features)   # (N, 4, H, W)
            ctr_logits = self.pred_ctr(box_features)   # (N, 1, H, W)
            
            # Permute and flatten: (N, H, W, C) -> (N, H*W, C)

            class_logits[level] = cls_logits.permute(0, 2, 3, 1).contiguous().view(cls_logits.shape[0], -1, self.num_classes)
            boxreg_deltas[level] = box_deltas.permute(0, 2, 3, 1).contiguous().view(box_deltas.shape[0], -1, 4)
            centerness_logits[level] = ctr_logits.permute(0, 2, 3, 1).contiguous().view(ctr_logits.shape[0], -1, 1)
...
@torch.no_grad() #  不需要计算的地方禁用梯度 节约性能花费
def fcos_match_locations_to_gt(
    ...
    # 这里提示下自己 h*w 格确实就足够存储 但是向量化成这个 shape 的矩阵效率高 也省得转换
    Args:
    ...
    for level_name, centers in locations_per_fpn_level.items():
        ...
        x, y = centers.unsqueeze(dim=2).unbind(dim=1) # unqueeze -> 插入指定的某个维度形成一个新维度 unbind : 沿着某个维度解开绑定的n个元素展开为n个 ， 但是维度空间减一就
        ...
        lower_bound = stride * 4 if level_name != "p3" else 0 # p_3 设置成0因为在这个层里 目标可能会很小 
        ...
        # 优先匹配上面积小的框 然后因为经常有重叠所以不考虑标签判断而是优先分配精确小面积  
...
def fcos_get_deltas_from_locations(
    ...
    deltas = torch.zeros_like(locations).repeat(1, 2)  # (N, 4)
    
    # For background boxes, return all -1
    bg_mask = (gt_boxes[:, :4] == -1).all(dim=1) # bg_mask 就是比方说object是云 那么蓝天就是不用定位的b
    deltas[bg_mask] = -1
    
    # For foreground boxes, compute LTRB deltas
    fg_mask = ~bg_mask # 前景 = 非背景
    if fg_mask.any():
        xc, yc = locations[fg_mask].unbind(dim=1)
        x1, y1, x2, y2 = gt_boxes[fg_mask, :4].unbind(dim=1)
        ...
        # Compute distances to each edge and normalize by stride
        deltas[fg_mask, 0] = (xc - x1) / stride  # left
        deltas[fg_mask, 1] = (yc - y1) / stride  # top  
        deltas[fg_mask, 2] = (x2 - xc) / stride  # right
        deltas[fg_mask, 3] = (y2 - yc) / stride  # bottom
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def fcos_apply_deltas_to_locations(
    ...
    ##########################################################################
    # Initialize output boxes tensor
    output_boxes = torch.zeros_like(deltas)
    
    # Get background mask (deltas are all -1)
    ...
    if fg_mask.any():
        ...
        # Get deltas and un-normalize by stride
        l, t, r, b = deltas[fg_mask].unbind(dim=1)
        l = l * stride
        t = t * stride
        r = r * stride
        b = b * stride
        ...
        # Clip negative deltas to 0 (center must be inside box)
        ...
        output_boxes[fg_mask, 0] = xc - l  # x1
        output_boxes[fg_mask, 1] = yc - t  # y1
        output_boxes[fg_mask, 2] = xc + r  # x2
        output_boxes[fg_mask, 3] = yc + b  # y2
    
    # Background boxes remain zeros (or could set to -1 if preferred)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    """
    # Initialize centerness with -1 (background value)
    ...
    if fg_mask.any():
        ...
        # Calculate centerness according to FCOS paper Equation 3
